...
from reportlab.lib.styles import getSampleStyleSheet  # Estilos de texto no PDF
...
def carregar_dados(arquivo_excel):
    try:
        ...
        funcionarios = pd.DataFrame(columns=["Nome", "CPF", "Cargo"])
    
    try:
        acidentes = pd.read_excel(arquivo_excel, sheet_name="ACIDENTES")
    except:
        acidentes = pd.DataFrame(columns=["Funcionario", "Data", "Tipo de Acidente", "Status"])
    
    try:
        ocorrencias = pd.read_excel(arquivo_excel, sheet_name="OCORRENCIAS")
    except:
        ocorrencias = pd.DataFrame(columns=["Descricao", "Data"])
...
def cadastrar_acidente(funcionarios, acidentes):
    ...
    if escolha not in funcionarios.index:
...
def calcular_metricas(acidentes):
    ...
    afastados = acidentes[acidentes['Status'].str.lower() == 'com afastamento']
...
def relatorio_mensal(acidentes):
    ...
    acidentes['Data'] = pd.to_datetime(acidentes['Data'], dayfirst=True, errors='coerce')
    return acidentes.groupby(acidentes['Data'].dt.month)['Funcionario'].count()

# Gera gráficos de acidentes
def gerar_graficos(acidentes):
    if acidentes.empty:
        print("❌ Nenhum acidente cadastrado para gerar gráficos.")
        return
    ...
    if not acidentes_por_mes.empty:
        ...
        print("❌ Não há dados válidos de datas para gerar o gráfico mensal.")
